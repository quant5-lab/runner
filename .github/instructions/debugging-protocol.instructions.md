...
- Find the *first* test that fails in the test run. Ignore all subsequent failures.

### **STEP 2: ISOLATE THE TEST**
- Construct the exact command to run *only* the single failing test. Use `--grep` for Playwright, or equivalent filters for other frameworks.
...
- Re-run the *exact same* isolated test command to prove the fix works.
