...
| **17** | **Codegen** | Dynamic period expressions in TA functions | ✅ FIXED | Compile-time constant periods now supported: variables (`varPeriod`), binary expressions (`7*2`), call expressions (`getPeriod()`, `input.int()`). `UserFunctionEvaluator` enables constant folding through user-defined functions. Runtime dynamic periods generate IIFE pattern for `ta.sma`, `ta.ema`, `ta.stdev`, `ta.highest`, `ta.lowest`. See `tests/period-expressions/` | test.pine |
