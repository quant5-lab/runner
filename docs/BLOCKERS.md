...
| **15** | **Codegen** | `security()` package import missing | FIXED | Recursive walker detects nested security() calls in conditionals/binary expressions | utbot-quantnomad.pine |
...
| **27** | **Runtime** | `timeframe.period` runtime resolution | VALID | Treated as literal string, fetcher looks for `BTCUSDT_timeframe.period.json` instead of resolving to ctx.Timeframe | utbot-quantnomad.pine |
