#!/usr/bin/env node
/**
 * E2E Test: Strategy with BULLISH mock data
 * Purpose: Verify LONG positions work correctly
 */
import { createContainer } from '../../src/container.js';
import { readFile } from 'fs/promises';
import { MockProviderManager } from '../mocks/MockProvider.js';

console.log('Testing strategy with BULLISH mock data...\n');

const mockProvider = new MockProviderManager({ dataPattern: 'bullish', basePrice: 100, amplitude: 10 });
...
const getVals = (title) => result.plots?.[title]?.data?.map(d => d.value).filter(v => v != null) || [];
...
console.log('\n=== EQUITY ===');
console.log('Range:               ', [Math.min(...equity).toFixed(0), Math.max(...equity).toFixed(0)]);
...
for (let i = 50; i < 56; i++) {
  ...
  const p = posSize[i] || 0;
  console.log(`${i.toString().padStart(3)} | ${c.padStart(8)} | ${s.padStart(8)} | ${l} | ${sh} | ${p.toString().padStart(7)}`);
}
