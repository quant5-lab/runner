...
async function runStrategyWithPattern(bars, strategyPath, pattern = 'linear', basePrice = 100, amplitude = 10) {
...
const fixnanResult = await runStrategyWithPattern(30, 'e2e/fixtures/strategies/test-fixnan.pine', 'linear');
...
const pivotResult = await runStrategyWithPattern(30, 'e2e/fixtures/strategies/test-pivothigh.pine', 'sawtooth', 100, 10);
...
let pivotMatched = 0, pivotTotal = 0;
...
console.log(`✅ pivothigh: ${pivotMatched}/${pivotTotal} match (found ${pivot2Count} pivot2, ${pivot5Count} pivot5)`);

// Test pivotlow with sawtooth pattern
const pivotlowResult = await runStrategyWithPattern(30, 'e2e/fixtures/strategies/test-pivotlow.pine', 'sawtooth', 100, 10);
...
let pivotlowMatched = 0, pivotlowTotal = 0;
...
console.log(`✅ pivotlow: ${pivotlowMatched}/${pivotlowTotal} match (found ${pivotlow2Count} pivot2, ${pivotlow5Count} pivot5)`);

// Test valuewhen
const valuewhenResult = await runStrategyWithPattern(50, 'e2e/fixtures/strategies/test-valuewhen.pine', 'linear');
...
console.log(`✅ valuewhen: ${valuewhenMatched}/${vw0Values.length + vw1Values.length} values match`);

// Test barmerge constants
const barmergeResult = await runStrategyWithPattern(30, 'e2e/fixtures/strategies/test-barmerge.pine', 'linear');
...
// Test time()
const timeResult = await runStrategyWithPattern(30, 'e2e/fixtures/strategies/test-time.pine', 'linear');
...
console.log(`✅ time: Daily ${validDailyTimes}/${timeDailyValues.length}, Weekly ${validWeeklyTimes}/${timeWeeklyValues.length} valid timestamps`);
