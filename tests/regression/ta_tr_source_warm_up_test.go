...
func TestTrBareIdentifier_SmaWarmUp(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			strategy := fmt.Sprintf("//@version=5\nindicator(\"TR Bare SMA Test\")\nplot(ta.sma(tr, %d), \"SMA\")\n", tc.period)
			assertWarmUp(t, strategy, "SMA", tc.period, syntheticTRValue)
		})
	}
}

/*
TestTrSource_EmaValidityBeyondSeed verifies that ta.ema(ta.tr, period) produces
valid values for multiple consecutive bars starting at bar period.
...
(Pine: na(sum[1]) ? src), so the first valid output at bar period equals the current
ta.tr value. On synthetic data where ta.tr is constant 200 for bars ≥ 1, the recovered
value is exactly syntheticTRValue, and all subsequent bars remain at that value.
*/
func TestTrSource_EmaValidityBeyondSeed(t *testing.T) {
	...
	const period = 5
	strategy := fmt.Sprintf("//@version=5\nindicator(\"TR EMA Sustained\")\nplot(ta.ema(ta.tr, %d), \"EMA\")\n", period)
	vals := runIndicator(t, strategy, "EMA", period+5)
	...
	for bar := period; bar < period+4 && bar < len(vals); bar++ {
		assertBarValue(t, "EMA", vals, bar, syntheticTRValue)
	}
}
...
func TestTrSource_SmaWarmUp(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			assertTrSourceWarmUp(t, tc.period, buildTrSmaPineScript(tc.period))
		})
	}
}
...
func TestTrSource_EmaWarmUp(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			assertTrSourceWarmUp(t, tc.period, buildTrEmaPineScript(tc.period))
		})
	}
}

/*
TestTrSource_RmaNaPropagation verifies that ta.rma(ta.tr, period) produces NaN
for all bars, not just the warm-up period.

ta.rma seeds its state with ta.sma(source, period) on bar period-1. For ta.tr
source, that window spans bars 0..period-1 and always includes bar 0 where
ta.tr = NaN (handle_na=false, no previous close). NaN in the seed sum propagates
to a NaN seed, and the recursive formula rma = alpha*source + (1-alpha)*prev_rma
perpetuates NaN whenever prev_rma is NaN. This matches Pine's behavior.
*/
func TestTrSource_RmaNaPropagation(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping integration test in short mode")
	}
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			assertAllNaN(t, buildTrRmaPineScript(tc.period), "ATR", tc.period+5)
		})
	}
}
...
func TestTrSource_WarmUpBoundaryContrast(t *testing.T) {
	...
	// ta.sma(ta.tr, period): bar period-1 still in warm-up, bar period is first valid
	...
	// ta.atr(period): bar period-1 is the first valid bar (handle_na=true seed covers bar 0)
...
func assertAllNaN(t *testing.T, strategy, indicatorName string, barCount int) {
	t.Helper()
	vals := runIndicator(t, strategy, indicatorName, barCount)
	for bar, v := range vals {
		if !math.IsNaN(v) {
			t.Errorf("bar %d: expected NaN (perpetual na), got %.6f", bar, v)
		}
	}
}

func assertTrSourceWarmUp(t *testing.T, period int, strategy string) {
	t.Helper()
	assertWarmUp(t, strategy, "ATR", period, syntheticTRValue)
}
