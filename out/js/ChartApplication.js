...
import { TradeDataFormatter, formatCurrency } from './TradeTable.js';
...
export class ChartApplication {
  ...
  async initialize() {
    ...
    setTimeout(() => ChartManager.fitContent(this.paneManager.getAllCharts()), 50);
  }
  ...
  _loadTrades(strategy, candlestickData) {
    ...
    const rows = new TradeRowspanTransformer(formatter).transformTrades(allTrades, currentPrice);
    ...
    const unrealizedProfit = currentPrice
      ? (strategy.openTrades || []).reduce((sum, t) => {
