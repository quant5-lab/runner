...
for i := 0; i < len(ctx.Data); i++ {
	...
	crossover_signalSeries.Set(func() float64 { if (sma20Series.Get(0) > sma50Series.Get(0) && prev_sma20Series.Get(0) <= prev_sma50Series.Get(0)) { return 1.0 } else { return 0.0 } }())
	crossunder_signalSeries.Set(func() float64 { if (sma20Series.Get(0) < sma50Series.Get(0) && prev_sma20Series.Get(0) >= prev_sma50Series.Get(0)) { return 1.0 } else { return 0.0 } }())
	...
	if i > 0 {
		...
		ta_crossoverSeries.Set(func() float64 { if sma20Series.Get(0) > sma50Series.Get(0) && ta_crossover_prev1 <= ta_crossover_prev2 { return 1.0 } else { return 0.0 } }())
	} else {
	...
	if i > 0 {
		...
		ta_crossunderSeries.Set(func() float64 { if sma20Series.Get(0) < sma50Series.Get(0) && ta_crossunder_prev1 >= ta_crossunder_prev2 { return 1.0 } else { return 0.0 } }())
	} else {
		ta_crossunderSeries.Set(0.0)
	}
	manual_signalSeries.Set(func() float64 { if crossover_signalSeries.Get(0) != 0 { return 1.00 } else { return 0.00 } }())
	ta_signalSeries.Set(func() float64 { if ta_crossoverSeries.Get(0) != 0 { return 1.00 } else { return 0.00 } }())
	...
	if i < len(ctx.Data)-1 { sma20Series.Next() }
	if i < len(ctx.Data)-1 { sma50Series.Next() }
	if i < len(ctx.Data)-1 { prev_sma50Series.Next() }
	if i < len(ctx.Data)-1 { crossunder_signalSeries.Next() }
	if i < len(ctx.Data)-1 { ta_crossunderSeries.Next() }
	if i < len(ctx.Data)-1 { prev_sma20Series.Next() }
	if i < len(ctx.Data)-1 { crossover_signalSeries.Next() }
	if i < len(ctx.Data)-1 { ta_crossoverSeries.Next() }
	if i < len(ctx.Data)-1 { manual_signalSeries.Next() }
	if i < len(ctx.Data)-1 { ta_signalSeries.Next() }
}
