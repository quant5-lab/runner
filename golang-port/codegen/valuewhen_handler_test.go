...
func TestValuewhenHandler_GenerateCode_ValidCases(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expectSource:    "closeSeries.Get(lookbackOffset)",
			expectOccur:     "0",
		},
		{
...
func TestGenerator_ConvertSeriesAccessToOffset(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want:       "closeSeries.Get(lookbackOffset)",
		},
		{
			name:       "bar.High with offset",
			seriesCode: "bar.High",
			offsetVar:  "lookbackOffset",
			want:       "highSeries.Get(lookbackOffset)",
		},
		{
			name:       "bar.Low with offset",
			seriesCode: "bar.Low",
			offsetVar:  "offset",
			want:       "lowSeries.Get(offset)",
		},
		{
			name:       "bar.Open with offset",
			seriesCode: "bar.Open",
			offsetVar:  "o",
			want:       "openSeries.Get(o)",
		},
		{
			name:       "bar.Volume with offset",
			seriesCode: "bar.Volume",
			offsetVar:  "lookbackOffset",
			want:       "volumeSeries.Get(lookbackOffset)",
		},
		{
