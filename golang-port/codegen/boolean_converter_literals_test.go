...
func TestBooleanConverter_SeriesAccessPatterns(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expectedOp:  "(signalSeries.GetCurrent() != 0)",
		...
		},
		{
			...
			expectedOp:  "(previousSeries.Get(1) != 0)",
			expectedIf:  "previousSeries.Get(1) != 0",
		...
		},
		{
			...
			expectedOp:  "(pastSeries.Get(5) != 0)",
			expectedIf:  "pastSeries.Get(5) != 0",
		...
		},
		{
			...
			expectedOp:  "(ta.Sma(closeSeries.GetCurrent(), 20) != 0)",
		...
		},
		{
			...
			expectedOp:  "(highSeries.GetCurrent() - lowSeries.GetCurrent() != 0)",
			expectedIf:  "highSeries.GetCurrent() - lowSeries.GetCurrent() != 0",
...
func TestBooleanConverter_ComplexExpressions(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expected:    "(signalSeries.GetCurrent() != 0 ? 1.0 : 0.0 != 0)",
		...
		},
		{
			...
			expected:    "(priceSeries.GetCurrent() * 1.5 + 10 != 0)",
