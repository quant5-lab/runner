...
func TestOHLCVFieldAccessGenerator_WithOffset(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wantLoopAccessPattern: "closeSeries.Get(j)",
		},
		{
			...
			baseOffset:            1,
			period:                20,
			wantInitialAccess:     "closeSeries.Get(20)",
			wantLoopAccessPattern: "closeSeries.Get(j+1)",
		},
		{
			...
			wantLoopAccessPattern: "closeSeries.Get(j+4)",
		},
		{
			...
			wantLoopAccessPattern: "highSeries.Get(j+10)",
		},
		{
			...
			baseOffset:            2,
			period:                14,
			wantInitialAccess:     "lowSeries.Get(15)",
			wantLoopAccessPattern: "lowSeries.Get(j+2)",
		},
		{
			...
			baseOffset:            100,
			period:                1,
			wantInitialAccess:     "volumeSeries.Get(100)",
			wantLoopAccessPattern: "volumeSeries.Get(j+100)",
		},
		{
			...
			baseOffset:            0,
			period:                5,
			wantInitialAccess:     "openSeries.Get(4)",
			wantLoopAccessPattern: "openSeries.Get(j)",
		},
		{
			...
			baseOffset:            3,
			period:                10,
			wantInitialAccess:     "volumeSeries.Get(12)",
			wantLoopAccessPattern: "volumeSeries.Get(j+3)",
		},
	}
...
func TestCreateAccessGenerator_WithOffset(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			period:            10,
			wantInitialAccess: "highSeries.Get(9)",
			wantLoopAccess:    "highSeries.Get(j)",
		},
		{
...
func TestAccessGenerator_OffsetCalculation(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Extract the offset from generated code: "closeSeries.Get(X)"
